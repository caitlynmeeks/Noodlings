...
class InspectorPanel(QWidget):
    ...
    def load_zone_properties(self, zone_data):
        ...
        uuid_widget = QLabel(f'{display_id} <a href="#" style="color: #666;">[copy]</a>')
    ...
    def load_noodling_properties(self, entity_data):
        ...
        try:
            ...
            print(f"[Inspector] load_noodling_properties DEBUG:")
            print(f"[Inspector]   noodling_ref = '{noodling_ref}'")
            print(f"[Inspector]   instance_path = '{instance_path}'")
            print(f"[Inspector]   entity_data keys = {list(entity_data.keys())}")

            if noodling_ref and instance_path:
                # Project mode: Load from Library/Noodlings/{noodling_ref}/recipe.yaml
                # Instance path is like: Project/Stages/StageName/Instances/uuid
                # Need to go up 4 levels: uuid->Instances->StageName->Stages->Project
                ...
                print(f"[Inspector] Looking for recipe at: {library_recipe}")
        ...
        basics_group = self.create_property_group("Noodling Properties")
        ...
        # UUID (read-only) with copy button - consistent with Object Properties
        ...
        # This is the ONLY affect representation - NO discrete emotion labels
        ...
        # ===== LLM CONFIGURATION =====
        # Check llm.model (nested) first, then fallback to top-level model/llm_model
    ...
    def load_object_properties(self, entity_data):
        ...
        # UUID (read-only) with copy button - consistent with zones/noodlings
        display_id = obj_id
        if len(display_id) > 20:
            display_id = display_id[:8] + "..." + display_id[-4:]
        uuid_widget = QLabel(f'{display_id} <a href="#" style="color: #888;">[copy]</a>')
        ...
        uuid_widget.setWordWrap(False)
        uuid_widget.setTextInteractionFlags(Qt.TextInteractionFlag.TextBrowserInteraction)
        uuid_widget.linkActivated.connect(lambda: QApplication.clipboard().setText(obj_id))
        uuid_widget.setToolTip(f"Full ID: {obj_id}\nClick [copy] to copy")
        obj_group.content.layout().addRow("UUID:", uuid_widget)
    ...
    # ========== ASSET INSPECTION (from Assets panel) ==========

    def load_asset_properties(self, entity_data: dict):
        """
        Load asset properties into inspector based on asset sub-type.

        Called when user selects an asset in the Assets panel.
        Dispatches to specific loader based on file type.
        ...
        if asset_type == 'folder':
            self._load_asset_folder(path)
        elif asset_type == 'noodling':
            self._load_asset_noodling(path)
        elif asset_type == 'stage':
            self._load_asset_stage(path)
        elif asset_type == 'radiance':
            self._load_asset_radiance(path)
        elif asset_type == 'vrm':
            self._load_asset_vrm(path)
        elif asset_type == 'image':
            self._load_asset_image(path)
        elif asset_type == 'audio':
            self._load_asset_audio(path)
        elif asset_type == 'script':
            self._load_asset_script(path)
        elif asset_type == 'neural_canvas':
            self._load_asset_neural_canvas(path)
        elif asset_type in ('yaml', 'zone'):
            self._load_asset_yaml(path)
        else:
    ...
    def _load_asset_noodling(self, path: str):
        ...
        # Find the recipe.yaml in the folder
        ...
        if personality:
            pers_group = self.create_property_group("Personality")

            # Big 5 traits
            traits = ['openness', 'conscientiousness', 'extraversion', 'agreeableness', 'neuroticism']
            for trait in traits:
                value = personality.get(trait, 0.5)
                self.add_text_field(pers_group, trait.title(), f"{value:.2f}", read_only=True)

            self.properties_layout.addWidget(pers_group)
        ...
        # Rez button
        rez_btn = QPushButton("Rez")
        rez_btn.setStyleSheet(self._action_button_style())
        rez_btn.clicked.connect(lambda: self._rez_noodling(path))
        actions_layout.addWidget(rez_btn)

        # Edit Recipe button
        edit_btn = QPushButton("Edit Recipe")
        edit_btn.setStyleSheet(self._action_button_style())
        edit_btn.clicked.connect(lambda: self._open_in_system(recipe_path))
    ...
    def _load_asset_stage(self, path: str):
        ...
        # Find the stage.yaml in the folder
        if os.path.isdir(path):
        ...
        # Count zones and instances from stage folder
        ...
        open_btn = QPushButton("Open Stage")
        open_btn.setStyleSheet(self._action_button_style())
        open_btn.clicked.connect(lambda: self._open_stage(path))
    ...
    def _load_asset_radiance(self, path: str):
        ...
        try:
            ...
            self.add_text_field(group, "File Size", size_str, read_only=True)
        except:
            pass
        ...
        try:
            ...
            if hasattr(asset, 'positions') and asset.positions is not None:
                gaussian_count = len(asset.positions)
                self.add_text_field(stats_group, "Gaussians", f"{gaussian_count:,}", read_only=True)
            ...
            if hasattr(asset, 'clip_embeddings') and asset.clip_embeddings is not None:
                self.add_text_field(stats_group, "CLIP Embeddings", "Yes", read_only=True)
            else:
                self.add_text_field(stats_group, "CLIP Embeddings", "No", read_only=True)
        ...
        view_btn = QPushButton("Open in Viewer")
        view_btn.setStyleSheet(self._action_button_style())
        view_btn.clicked.connect(lambda: self._open_in_gaussian_viewer(path))
    ...
    def _load_asset_vrm(self, path: str):
        ...
        import_btn = QPushButton("Import as Radiance")
        import_btn.setStyleSheet(self._action_button_style())
        import_btn.clicked.connect(lambda: self._import_vrm_as_radiance(path, densify_cb.isChecked(), face_cb.isChecked(), scale_spin.value()))
        actions_layout.addWidget(import_btn)

        preview_btn = QPushButton("Preview")
        preview_btn.setStyleSheet(self._action_button_style())
        preview_btn.clicked.connect(lambda: self._preview_vrm(path))
    ...
    def _load_asset_image(self, path: str):
        ...
        # Dimensions (try to load with PIL or Qt)
        try:
            pixmap = QPixmap(path)
        ...
        open_btn.setStyleSheet(self._action_button_style())
        open_btn.clicked.connect(lambda: self._open_in_system(path))
        actions_layout.addWidget(open_btn)
    ...
    def _load_asset_audio(self, path: str):
        ...
        play_btn.setStyleSheet(self._action_button_style())
    ...
    def _load_asset_script(self, path: str):
        ...
        # Code preview (scrollable, read-only)
        ...
        try:
            with open(path, 'r') as f:
                content = f.read(8192)  # Limit to 8KB for preview
                if len(content) >= 8192:
                    content += "\n\n... (truncated)"
        ...
        edit_btn = QPushButton("Open in Editor")
        edit_btn.setStyleSheet(self._action_button_style())
        edit_btn.clicked.connect(lambda: self._open_in_system(path))
        actions_layout.addWidget(edit_btn)
    ...
    def _load_asset_neural_canvas(self, path: str):
        ...
        edit_btn = QPushButton("Open in Editor")
        edit_btn.setStyleSheet(self._action_button_style())
        edit_btn.clicked.connect(lambda: self._open_neural_canvas(path))
        actions_layout.addWidget(edit_btn)
    ...
    def _rez_noodling(self, path: str):
        ...
        # Signal back to main window to handle rezzing
        main_window = self.window()
    ...
    def _preview_vrm(self, path: str):
        ...
        if hasattr(main_window, 'vrm_preview'):
            ...
            # Switch to VRM Preview tab if it exists
